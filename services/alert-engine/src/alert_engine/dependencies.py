"""FastAPI dependencies for the Alert Engine."""
...
ROLES_WITH_ADMIN_ACCESS: frozenset[str] = frozenset({"it_administrator"})
...
async def require_write_access(request: Request) -> None:
    ...
    if role not in ROLES_WITH_WRITE_ACCESS:
        raise HTTPException(
            ...
            detail=f"Role '{role}' does not have write access to the Alert Engine",
        )


async def require_admin(request: Request) -> None:
    role: str = request.state.role
    if role not in ROLES_WITH_ADMIN_ACCESS:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Role '{role}' does not have admin access to the Alert Engine",
        )
